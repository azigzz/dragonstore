...
export default function CartDrawer({
  ...
  async function copyOrder() {
    ...
    try {
      ...
      setMessage("Pedido copiado! Agora envie no Discord para finalizar.");
    } catch {
        ...
        {open ? (
          <>
            ...
            <motion.aside
              ...
              className="fixed right-0 top-0 z-50 flex h-dvh w-full max-w-md flex-col border-l border-white/10 bg-[#0b0f18] shadow-2xl"
              aria-label="Carrinho"
            >
              ...
              <div className="scrollbar-thin flex-1 space-y-3 overflow-y-auto p-4">
                {items.length ? items.map(item => (
                  <div key={item.product.id} className="premium-surface rounded-lg border border-white/10 p-3">
                  ...
                  <div className="rounded-lg border border-dashed border-white/15 p-5 text-sm text-slate-400">
                    Seu carrinho esta vazio. Escolha um produto para montar o pedido e finalizar pelo Discord.
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3 border-t border-white/10 p-4">
                <div className="rounded-lg border border-emerald-300/20 bg-emerald-300/10 p-3">
                  ...
                  <p className="mt-2 text-xs leading-5 text-slate-300">
                    Copie o pedido e envie no Discord para a equipe confirmar pagamento e entrega.
                  </p>
                </div>
                ...
                <button
                  ...
                  disabled={!items.length}
                  className="pulse-cta inline-flex h-12 w-full items-center justify-center gap-2 rounded-md bg-emerald-300 px-4 text-sm font-black text-black transition hover:bg-cyan-200 disabled:cursor-not-allowed disabled:opacity-45 disabled:[animation:none]"
                >
