...
Para os comandos com prefixo `!` funcionarem, ative **Message Content Intent** em `Developer Portal -> Bot -> Privileged Gateway Intents`. Para Aiven/Postgres, mantenha `DATABASE_SSL=true`; a URI pode usar `sslmode=require`. Se o endpoint exigir handshake TLS direto, defina tambem `DATABASE_DIRECT_TLS=true`.
